...
Linking an item to other items in the library across providers occurs when the item is added to the MA library. It can also be triggered manually by using the ![icon](assets/icons/database-search.png){ width=20 }icon for an item. This allows a library item to be linked to their equivalent items across providers without the other items be added to the library. This process [does not currently happen automatically](https://github.com/orgs/music-assistant/discussions/4894#discussioncomment-15736163), and there is no way to trigger it as a bulk operation.
