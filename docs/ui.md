...
The creation of links within the MA library to other identical items within or across providers can occur when an item is added to the MA library. Links will be made automatically if internal matching logic is satisfied. Links can also be created manually by pressing this icon ![icon](assets/icons/database-search.png){ width=20 } when seen beside an item. There is no way to trigger the linking of items as a bulk operation. Adding a new provider does not trigger linking of existing library items to items available on that provider except for items in the new providers library (however called) and if the `Sync library <item>  from this provider to Music Assistant` option is on.
