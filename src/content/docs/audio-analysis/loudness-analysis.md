...
It is built in, always on, and cannot be disabled. Measurements run automatically on tracks that do not yet have one stored. Scanning is done as a background task for local file system (i.e. Local, SMB or NFS) provided tracks and all in-library tracks are scanned during playback.
