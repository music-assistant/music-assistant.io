...
6. Currently in order for the callback to work MA must be accessed via the exposed webserver port when setting up this source. If MA has been installed as an app, then the port must be manually exposed as described in the [core settings](/settings/core/#webserver-frontend-and-api). Thus the URL when setting this source up must be `http://<YOUR_MA_IP>:8095`. Once successfully configured the webserver port can be disabled again if desired.
