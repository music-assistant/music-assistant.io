...
When an administrator adds a source, it starts out shared with everyone, and they can then make it their own or a member's. Members can also add their own account for a service that can be added more than once, which only they can use until they share it. See [Personal and shared music sources](/settings/user-management/#personal-and-shared-music-sources).
