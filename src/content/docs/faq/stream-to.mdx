...
description: Find the Music Assistant player provider that can drive the device you want to play on
---

import TilePage from '../../../components/TilePage.astro';
import PlayerCapabilities from '../../../components/PlayerCapabilities.astro';
import { PLAYERS_PAGE } from '../../../data/players';

# I Want To Stream To

Music Assistant plays to devices through player providers, and there is one for each kind of
device or protocol. Start from the sort of thing you want to play on and pick the provider that
drives it.

<TilePage page={PLAYERS_PAGE} />
...
If a device supports several protocols they are combined into one player, shown as chips near the
player name in the settings. In the [Individual Player Settings](/settings/individual-player/#output-protocols)
you can set the default protocol, or disable any you do not use.

## Working out a specific setup

The rest of this page covers situations that need more than picking a provider, such as gear you
already own, a phone, a browser, or something you have yet to buy.
...
The native solution for MA is to use [Sendspin](https://www.sendspin-audio.com/). The device requires at least 2MB of PSRAM and 4MB of flash memory. Flash the device with [ESPHome](https://esphome.io/components/media_source/sendspin/). A number of more off the shelf items have pre-made software available on [RealDeco's GitHub repository](https://github.com/RealDeco/sendspin-guition) 
...
The <a href="https://github.com/trudenboy/sendspin-bt-bridge" target="_blank" rel="noopener noreferrer">Sendspin Bluetooth Bridge</a> is the most direct route. It runs on any Linux machine with a Bluetooth adapter, built in or a USB dongle, and every speaker or set of headphones you pair with it turns up in Music Assistant as a [Sendspin](/player-support/sendspin/) player. It can hold several devices at once, and because they are Sendspin players they stay in step with each other and with the rest of your speakers. It installs as a Home Assistant App, in Docker, or in an LXC container, and has been tested on Home Assistant OS, Ubuntu, Proxmox, OpenWrt and the Raspberry Pi 4 and 5. It is a community project rather than part of Music Assistant, so raise anything you need help with on its own repository.

Alternatively, if you have a spare Raspberry Pi (any model) then <a href="https://www.picoreplayer.org" target="_blank" rel="noopener noreferrer">PiCorePlayer</a> can also connect to Bluetooth speakers. To stream to your Bluetooth device using piCorePlayer, follow these best practices to ensure high-fidelity audio and a stable connection:
...
For such a project you need an audio-capture and ADC (analogue-to-digital converter) device that provides audio-input and digitalization. For example, you can use either a USB Audio Device Interface adapter from <a href="https://www.behringer.com/en/products?category_id=audio-interfaces--usb-audio-interfaces" target="_blank" rel="noopener noreferrer">Behringer</a> or <a href="https://www.ikmultimedia.com/products/irigstream/" target="_blank" rel="noopener noreferrer">IK Multimedia</a>, or a <a href="https://www.hifiberry.com/blog/need-some-input/" target="_blank" rel="noopener noreferrer">HiFiBerry board with ADC</a>.
