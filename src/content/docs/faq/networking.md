...
- **Reverse proxy.** A server that sits in front of a web application, usually to add HTTPS or a friendly address (for example `music.example.com` instead of an IP address and port number). Only relevant to advanced setups for accessing the MA interface — players always need direct access to the server. <a href="https://www.cloudflare.com/learning/cdn/glossary/reverse-proxy/" target="_blank" rel="noopener noreferrer">Learn more</a>
