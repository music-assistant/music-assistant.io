...
For streaming sources ONLY the SPECIFIC artists/albums/tracks/playlists that are in the streaming sources library (or favourites or however it is termed in the source) will be imported* into the MA library when the source is added and at each sync. This means, for example, if you have an artist in the sources library but none of their albums then all you will see in the MA library is the artist with NO associated albums or tracks. You have to subsequently add albums or tracks to the MA library if you want to see them in the library views. Note you can toggle the library / streaming source filter option to see all that is available in the streaming source.
...
As a further means of filtering the library, items can be marked as a "favourite". This is shown in the UI as a filled heart icon. Whether items are favourited by default when imported from the music source is determined by the [source settings](/music-providers/). You can see all items if you deselect the heart icon in the top menu.
