...
So you do not add speakers one at a time. You add the provider for the kind of device you own and it finds them for you. Most providers keep discovering, so a speaker you plug in next month turns up on its own.

Some devices can be reached more than one way. Anything Home Assistant already knows about can be played to through [Home Assistant Media Players](/player-support/ha/), but where Music Assistant has a provider of its own for that device, the native one is the better choice — which is what [Audio Quality](#audio-quality) below is about.
